    ...
    <div class="grid gap-4 xl:grid-cols-2">
        @forelse ($users as $user)
            ...
            <div>
                ...
                @unless ($isSelf)
                    <form method="POST" action="{{ route('admin.users.destroy', $user) }}" data-confirm="Delete the user “{{ $user->username }}”? They’ll no longer be able to sign in." class="mt-1 text-right">
                        @csrf @method('DELETE')
                        <button type="submit" class="text-xs text-red-600 hover:underline">Delete This User</button>
                    </form>
                @endunless
            </div>
        @empty
